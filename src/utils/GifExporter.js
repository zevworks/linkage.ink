...
export class GifExporter {
  ...
  captureFrame(canvas, currentAngle) {
    ...
    // Skip every 3rd frame (capture 20fps instead of 60fps)
    if (this.captureCallCount % 3 !== 0) {
      // Still need to check if we've reached the end
      if (this.captureCallCount >= this.targetFrames) {
    ...
    // Add frame for smooth playback
    try {
      // Get the canvas element (p5.js wraps it)
      const canvasElement = canvas.elt || canvas;
      this.gif.addFrame(canvasElement, { delay: 48, copy: true }); // 20fps (48ms = 1000/20)
    } catch (error) {
